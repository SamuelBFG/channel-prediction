...
def compile_and_fit(model, window, max_epochs, patience=10, file_name="models", save_weights_only=False):
  ...
  # print('COMECEI A PRINTAR')
  # print(history.history)  
  # print('ACABEI DE PRINTAR')
...
def plot_errors(mae_dict, rmse_dict, maeRT_dict, rmseRT_dict, model_name, INPUT_WIDTH=s.INPUT_WIDTH, OUT_STEPS=s.OUT_STEPS, CFG_L1=s.CFG_L1, CFG_L2=s.CFG_L2, show_plots=s.SHOW_PLOTS):
  ...
  # figures_dir = s.FIGURES_DIR+str(model_name)+'_input_'+str(INPUT_WIDTH)+'_output_'+str(OUT_STEPS)
  ...
  if len(CFG_L2) > 0:
    df = pd.DataFrame(dt, index=['{} - {} units'.format(x,y) for x,y in zip(CFG_L1, CFG_L2)])
  else:
    df = pd.DataFrame(dt, index=[str(x) + ' units' for x in CFG_L1])
...
#validation dataset
  ...
  # print('DEBUGGING PURPOSES: TEST ERROR DICT:')
  # print(test_performance_rt)
...
def save_parameters(model_name, INPUT_WIDTH, OUT_STEPS, CFG_L1, CFG_L2, DROPOUT, MAX_EPOCHS, figures_dir):
...
def fitLSTM(window, train_min, train_max, INPUT_WIDTH=s.INPUT_WIDTH, OUT_STEPS=s.OUT_STEPS, CFG_L1=s.CFG_L1, CFG_L2=s.CFG_L2, DROPOUT=s.DROPOUT, MAX_EPOCHS=s.MAX_EPOCHS, show_plots=s.SHOW_PLOTS):
  ...
  save_parameters(model_name, INPUT_WIDTH, OUT_STEPS, CFG_L1, CFG_L2, DROPOUT, MAX_EPOCHS, figures_dir)
...
def fitLinearRegression(window, train_min, train_max, INPUT_WIDTH=s.INPUT_WIDTH, OUT_STEPS=s.OUT_STEPS, MAX_EPOCHS=s.MAX_EPOCHS, show_plots=s.SHOW_PLOTS):
  ...
  save_parameters(model_name, INPUT_WIDTH, OUT_STEPS, 0, 0, 0, MAX_EPOCHS, figures_dir)
